...
export default class Carousel extends React.Component {
  ...
  handleArrowClick(e) {
    ...
    this.autoplay();
    if (e.target.className === 'fa-solid fa-2xl fa-chevron-right right-arrow') {
      this.setState({ currentImage: this.state.currentImage + 1 });
      if (this.state.currentImage >= this.props.images.length - 1) {
        this.setState({ currentImage: 0 });
      }
    }
    if (e.target.className === 'fa-solid fa-2xl fa-chevron-left left-arrow') {
      this.setState({ currentImage: this.state.currentImage - 1 });
  ...
  handleCircleClick(e) {
    clearInterval(this.state.intervalId);
    this.autoplay();
    const circleNumber = Number(e.target.id);
    if (e.target.className === 'fa-regular fa-lg fa-circle two circle-icon') {
